...
common_files_test['efficiency_ramp'] = efficiency_ramp_test

# hold common files parsed dataframes
common_files_parsed = {}

common_files_parsed['saturation'] = (
    stockturnover.create_common_files_dataframe(
        stock_object.saturation, 
        saturation_test.copy()
    )
)

common_files_parsed['ramp_efficiency'] = (
    stockturnover.create_common_files_dataframe(
        stock_object.ramp_efficiency, 
        efficiency_ramp_test.copy()
    )
)
...
class TestCreateCommonFilesDataFrame():
    ...
    def test_create_common_files_interpolation(self):
        ...
        parsed_interp = np.concatenate(
            [common_files_parsed[i][params[i]['label']].values for i in common_files_parsed.keys()]
        )
