...
from enduse.stockobjects import CalcConfig, StockObject, CommonStockObject, BuildingObject

class StockTurnoverModel():

    def __init__(
        self, 
        stock_objects: Type[StockObject],
        calc_config: Type[CalcConfig]
    ):
    ...
    def run_model(
        ...
        # run model
        model_data = self.join_stock_objects(
            self.build_join_order(stock_objects)
        )

        model_data = model_data.sort_values(
            by=list(calc_config.equipment_sort_index)
        )
        ...
        for i in stock_objects.__fields__:            
            if i == 'buildings':
                setattr(
                    self, 
                    i, 
                    create_buildings_dataframe(
                        getattr(stock_objects, i)
                    )
                )            
            else:
                setattr(
...
def create_common_files_dataframe(
    stock_object: Type[CommonStockObject],
    ...
    df[stock_object.start_time] = pd.to_datetime(df[stock_object.start_time], format='%Y')
    df[stock_object.end_time] = pd.to_datetime(df[stock_object.end_time], format='%Y')

    # create standard date ranges
    dates = [
        pd.date_range(r[0], r[1], freq='AS') for r in 
        df[[stock_object.start_time, stock_object.end_time]].values
    ]
...
def create_buildings_dataframe(
    stock_object: Type[BuildingObject],
    df: pd.DataFrame = None
) -> pd.DataFrame:
